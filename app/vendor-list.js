...
const fixedVendors = []; // Removed dummy data
...
export default function VendorListScreen() {
  ...
  const loadVendors = async (isLoadMore = false) => {
    ...
    try {
      ...
      setError(
        `Failed to load vendors. Error: ${err.message || "Network Error"}`,
      );
      if (!isLoadMore) setVendors([]);
    } finally {
      setLoading(false);
  ...
  const handleScroll = (event) => {
    ...
    if (
      ...
      !loading &&
      !loadingMore &&
      vendors.length < totalVendors
    ) {
      console.log(
        "DEBUG: Loading more vendors...",
        vendors.length,
        "/",
        totalVendors,
      );
  ...
  useEffect(() => {
    ...
    // Auto-refresh every 15 seconds to show new vendors automatically
  ...
  const statuses = ["All", "New", "Approved", "Rejected", "Pending"];
  ...
  const filteredVendors = (vendors || []).filter((vendor) => {
    const matchesSearch =
      ...
      (vendor.AddressLine1 || "")
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      (vendor.Town || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
                ...
                >
                  ...
                  >
                    <Ionicons
                      ...
                      color="#EC4899"
                    />
                    <Text style={styles.shortcutText}>Retailer Details</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={styles.shortcutItem}
                    activeOpacity={0.7}
      ...
      <View style={styles.mainLayoutContainer}>
                    ...
                    <Pressable
                      ...
                      onPress={() =>
                        router.push({
                          ...
                          params: { id: vendor.VendorId.toString() },
                        })
                      }
                    >
                      <View style={styles.cardContent}>
                        ...
                        >
                          <Text style={styles.avatarText}>
                            {getInitial(vendor.VendorName || "V")}
                          </Text>
                        </LinearGradient>
                        <View style={styles.details}>
                            ...
                            {(() => {
                              let status_item =
                                ...
                                (filterStatus !== "All"
                                  ? filterStatus
                                  : "Pending");
                              if (status_item === "Auto-Approved")
                                status_item = "New";
                              const status_styles =
                                getStatusStyles(status_item);
                              return (
                                <View
                                  style={[
                                    styles.statusBadge,
                                    {
                                      backgroundColor: status_styles.bg,
                                      borderColor: status_styles.border,
                                    },
                                  ]}
                                >
                                  <Text
                          ...
                          <View style={styles.locationRow}>
                            ...
                            <Text style={styles.locationText} numberOfLines={1}>
                              {`${vendor.AddressLine1 || ""}, ${vendor.Town || ""}, ${vendor.District || ""}`}
                            </Text>
                          </View>
        ...
        {isMenuOpen && (
          <View style={styles.sideMenu}>
            ...
            >
              <Text
                style={{ color: "#F472B6", fontSize: 14, fontWeight: "600" }}
              >
                Clear
              </Text>
            </TouchableOpacity>
